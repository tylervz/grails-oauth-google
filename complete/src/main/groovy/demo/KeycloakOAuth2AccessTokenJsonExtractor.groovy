...
/**
 * Copy of {@link com.github.scribejava.core.extractors.OAuth2AccessTokenJsonExtractor} except that SCOPE_REGEX
 * has been changed to allow for scope values with spaces in them.
 */
class KeycloakOAuth2AccessTokenJsonExtractor implements TokenExtractor<OAuth2AccessToken> {
    ...
    protected KeycloakOAuth2AccessTokenJsonExtractor() {
    }

    private static class InstanceHolder {

        private static final KeycloakOAuth2AccessTokenJsonExtractor INSTANCE = new KeycloakOAuth2AccessTokenJsonExtractor()
    }

    static KeycloakOAuth2AccessTokenJsonExtractor instance() {
        return InstanceHolder.INSTANCE
    }

    @Override
    OAuth2AccessToken extract(Response response) throws IOException {
        ...
        return createTokenFromResponse(body)
    }
    ...
    private OAuth2AccessToken createTokenFromResponse(String response) {
